...
class TaskGroupDebug extends TaskGroup {
	...
	compare(_expectedDetails, testName) {
		...
		const expectedDetails = {
			...
			result:
				typeof _expectedDetails.result === 'undefined'
					? []
		...
		if (_expectedDetails.resultArgument) {
			deepEqual(
				_expectedDetails.resultArgument,
				this.result,
				testName + ': result argument was as expected'
			)
		}
		...
		if (_expectedDetails.errorArgument) {
			errorEqual(
				_expectedDetails.errorArgument,
				this.error,
				testName + ': error argument was as expected'
			)
		}
	}
}
		...
		test('should work with async', function (done) {
			...
			const task = Task.create(function (complete) {
				...
				wait(delay, function () {
					...
					equal(
						task.status,
						'running',
						'status to be running as we are within the task'
					)
			...
			task.done(function (err, result) {
				...
				errorEqual(
					err,
					null,
					'the callback error to be null as we did not error'
				)
				...
				equal(
					task.status,
					'passed',
					'status to be passed as we are within the completion callback'
				)
			})
			...
			equal(
				task.status,
				'created',
				"status to be created as we haven't called run yet"
			)
			...
			equal(
				task.status,
				'pending',
				'status to be pending as we just called run, and execute tasks asynchronously'
			)
			equal(
				task.result,
				null,
				'result to be null as we just called run, and execute tasks asynchronously'
			)
